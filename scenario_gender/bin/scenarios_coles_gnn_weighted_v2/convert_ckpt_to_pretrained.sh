                ...
                for GNN_LOSS_ALPHA in "${GNN_LOSS_ALPHA_OPTIONS[@]}"; do
                        for LP_CRITERION in "${LP_CRITERION_OPTIONS[@]}"; do
                            for CKPT_EPOCH in "${CKPT_EPOCHS[@]}"; do
                                ...
                                PYTHONPATH=.. python -m ptls_extension_2024_research.convert_coles_gnn_ckpt_to_pretrained_ckpt \
                                --config-dir conf --config-name coles_gnn_end2end_params_full_graph \
                                +ckpt_path="./checkpoints/${MODEL_NAME}/epoch\=${CKPT_EPOCH}.ckpt" \
                                +updated_ckpt_path="./checkpoints/converted_to_pretrained/${MODEL_NAME}/pretrain_epoches_${CKPT_EPOCH}.ckpt" \
                                +freeze_gnn_pretrained_layer=false \
