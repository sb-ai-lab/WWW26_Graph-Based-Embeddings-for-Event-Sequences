...
SPLIT_COUNT=2
MAX_EPOCHES=150
N_TRIPLETS_PER_ANCHOR_USER_OPTIONS=(128 1 32) 
MIN_ELEMENTS_IN_BIN_WHEN_ONE_BIN_ONLY_OPTIONS=(5)
LOSS_ALPHA_OPTIONS=(0.5 0.85 0.15 0 0.01) 
  ...
  for N_TRIPLETS_PER_ANCHOR_USER in "${N_TRIPLETS_PER_ANCHOR_USER_OPTIONS[@]}"; do
    for LOSS_ALPHA in "${LOSS_ALPHA_OPTIONS[@]}"; do
      for triplet_loss_margin in "${TRIPLET_LOSS_MARGIN_OPTIONS[@]}"; do
        ...
        experiment_name_without_max_epochs="coles_triplet__batch_size_${TRAIN_BATCH_SIZE}__split_count_${SPLIT_COUNT}__loss_alpha_${LOSS_ALPHA}__triplets_per_user_${N_TRIPLETS_PER_ANCHOR_USER}__bin_separation_margin_${MIN_ELEMENTS_IN_BIN_WHEN_ONE_BIN_ONLY}__triplet_loss_margin_${triplet_loss_margin}"
