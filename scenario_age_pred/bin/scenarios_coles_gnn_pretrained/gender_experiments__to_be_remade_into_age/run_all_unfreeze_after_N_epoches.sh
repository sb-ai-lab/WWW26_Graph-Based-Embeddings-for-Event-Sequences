...
for model_dir in "${!model_epoch_map[@]}"; do
  ...
  for epoch in "${EPOCHES[@]}"; do
    ...
    experiment_name="coles_gnn__pretrained_unfreeze_after_${N_EPOCHES_BEFORE_UNFREEZE}_epoches__${model_dir}__pretrain_epoches_${epoch}__epoches_${MAX_EPOCHES}"
    ...
    PYTHONPATH=.. python -m ptls_extension_2024_research.pl_train_module \
      ...
      pl_module.seq_encoder.trx_encoder.custom_embeddings.mcc_code.freeze="true" \
      ...
      +trainer.additional_callbacks="[{_target_:ptls_extension_2024_research.additional_callbacks.additional_callbacks.ModifyModelAfterNEpochesCallback,model_modifier:{_target_:ptls_extension_2024_research.additional_callbacks.model_modifiers.RequiresGradModifier,layers_getter:{_target_:ptls_extension_2024_research.additional_callbacks.layers_getters.CustomEmbedderGetter,custom_embedder_key:mcc_code},new_requires_grad_value:true},n_epoches_before_change:${N_EPOCHES_BEFORE_UNFREEZE}}]" \
