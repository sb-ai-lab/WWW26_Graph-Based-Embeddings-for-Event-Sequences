...
class InferenceModuleClientIdAware(InferenceModule):
    def forward(self, feats_and_client_ids: Tuple[PaddedBatch, torch.Tensor]):
        """
        Argument:
        feats_and_client_ids = Tuple[features: PaddedBatch, client_ids: torch.Tensor]
        """
        feats, client_ids = feats_and_client_ids
        out = self.model(feats_and_client_ids)
        if self.drop_seq_features:
            feats = feats.drop_seq_features()
            feats[self.model_out_name] = out
        else:
            feats.payload[self.model_out_name] = out
        if self.pandas_output:
            return self.to_pandas(feats)
        return feats
