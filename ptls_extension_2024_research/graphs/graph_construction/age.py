...
class AgeGraphBuilder(GraphBuilder):
    def preprocess(self, df, client_col, item_col):
        ...
        df['amount'] = df['amount_rur'].apply(abs)
        df['amount'] = np.log1p(df['amount'])
        df.drop(['amount_rur'], axis=1, inplace=True)
        return df
...
class AgeGraphBuildPipeline(GraphBuildPipeline):
    ENCODED_CLIENT_COL_NAME = 'encoded_client_id'
    graph_builder: GraphBuilder = AgeGraphBuilder()
