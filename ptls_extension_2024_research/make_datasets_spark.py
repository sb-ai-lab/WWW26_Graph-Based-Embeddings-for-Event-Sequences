...
class DatasetConverter:
    ...
    def _td_mts(self, df, col_date: str, col_part_of_day: str):
        """
        Arguments:
        ----------
        * `col_date`: str
            df[`col_date`] are `datetime.date` objects that contain year, mounth and day.
            `col_date` example: `datetime.date(2022, 6, 15)`
        * `col_part_of_day`: str
            df[`col_part_of_day`] are of type `str` and have four possible values:
            {'day', 'evening', 'morning', 'night'}

        This transform converts date like `datetime.date(2022, 6, 15)` to a float
        by converting it to unix timestamp; dividing by number of seconds in a day
        and adding a fraction of the day
    ...
    def remove_long_trx(self, df, max_trx_count, col_client_id):
        """
        Selects the last max_trx_count transactions
        """
    ...
    def load_transactions(self):
        ...
        if cols_event_time[0][0] == '#':
            ...
            elif cols_event_time[0] == '#mts':
                source_data = self._td_mts(source_data, cols_event_time[1], cols_event_time[2])
            else:
