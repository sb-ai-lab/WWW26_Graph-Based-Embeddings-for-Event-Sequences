...
for model_dir in "${!model_epoch_map[@]}"; do
  ...
  for pretrain_epoch in "${EPOCHES[@]}"; do
    ...
    experiment_name="coles_item_id_embed_init_with_pretrained_gnn__bs_${BATCH_SIZE}__split_count_${SPLIT_COUNT}__${model_dir}__pretrain_epoches_${pretrain_epoch}__epoches_${MAX_EPOCHES}"
